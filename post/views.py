...
class LikeView(UpdateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if user in post.likes.all():
            ...
            return JsonResponse({"status": "ok", "like_count": like_count, "class": "bi bi-heart"})

        else:
            post.likes.add(user)
            like_count = post.likes.count()
            return JsonResponse({"status": "ok", "like_count": like_count, "class": "bi bi-heart-fill"})
...
class PostDetailView(DetailView):
    ...
    def get_queryset(self):
        date = datetime(self.kwargs["year"], self.kwargs["month"], self.kwargs["day"]).date()
